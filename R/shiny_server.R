...
#' @import plotly
#' @import magrittr
#' @importFrom ggplot2 ggplot aes aes_string geom_histogram facet_grid geom_vline theme_light
...
shiny_server <- function(input, output) {
  ...
  observeEvent(input$start_sim,{
    ...
    reactVal$dt_sim_res <- est_srs(s_plot);
  });
  ...
  output$stand <- renderPlotly({
    ...
    plot_ly(data = pop()$data,
      ...
      add_markers(data = pop()$data, x = ~x_tree, y = ~y_tree,
                  visible = FALSE,
                  marker = list(size = pop()$data[, 10*scale(dbh, center = FALSE)],
                                sizemin = 5,
                                color = 'grey',
                                symbol = 'circle',
      ...
      layout(xaxis = ax, yaxis = ax,
             ...
             shapes = list(
               list(type = "path",
                    ...
                    path = xy_to_svg(sf::st_coordinates(pop()$boundary)[, 1:2]),
                    xref = "x", yref = "y")));
  });

  # renderDataTable is broken in shiny 1.4.0, a fiy will be in the next version
  # in the meantime use DT::renderDataTable instead of renderDataTable
  # https://github.com/rstudio/shiny/issues/2653
  output$estimates <- DT::renderDataTable({
    # avoid NOTEs by R CMD check
    y_hat <- NULL
    v_hat <- NULL
    est_appr <- NULL
    reactVal$dt_sim_res[,
                        list(y_hat = mean(y_hat),
                             v_emp = stats::var(y_hat),
                             v_hat = mean(v_hat)),
                        by = list(variable, est_appr)]
    });

  output$distribution <- renderPlot({
    A <- sum(sf::st_area(pop()$boundary))/10000;
    dt_y <- data.table(variable = input$select_var,
                       y = pop()$data[, sapply(.SD, sum), .SDcols = input$select_var]/A);
    ggplot(reactVal$dt_sim_res, aes(y_hat)) +
      geom_histogram() +
      facet_grid(est_appr ~ variable, scales = "free") +
      geom_vline(aes_string(xintercept = "y"), data=dt_y) +
      theme_light()
  });
}
...
get_data <- function(x) {
  e <- new.env();
  name <- utils::data(list = x, envir = e)[1];
  e[[name]];
}
